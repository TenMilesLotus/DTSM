...
def parse_layout(spans, num_rows, num_cols):  # sr, sc, er, ec
    layout = np.full([num_rows, num_cols], -1, dtype=int)
    cell_count = 0

    # for x1, y1, x2, y2 in spans:
    #     # layout[y1:y2+1, x1:x2+1] = cell_count
    #     layout[x1:x2+1, y1:y2+1] = cell_count
    #     cell_count += 1
    
    # only split
    for i in range(num_rows):
        for j in range(num_cols):
            layout[i][j] = cell_count
            cell_count += 1
...
def pred_result_to_table(table, pred_result):
    ...
    layout = parse_layout(spans, num_rows, num_cols)  # this
...
def table_to_relations(table):
    ...
    contents = ['77' for cell in table['cells']]
    relations = []
