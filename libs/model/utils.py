...
def extend_segments(row_segments, rows_es, col_segments, cols_es, cells_spans, layouts, divide_labels, merge_targets):
    ...
    for batch_idx in range(batch_size):
        ...
        merge_targets_pi = merge_targets[batch_idx]
        h, w = merge_targets_pi.shape
        ext_merge_targets_pi = np.full((len(ext_row_segments_pi) - 1, len(ext_col_segments_pi) - 1), -1)
        ext_merge_targets_pi[:h, :w] = merge_targets_pi.cpu().numpy()
        ...
        ext_merge_targets.append(ext_merge_targets_pi)

    return ext_row_segments, ext_col_segments, ext_cells_spans, aligned_layouts(ext_layouts, layouts), torch.tensor(ext_divide_labels).to(divide_labels.device), ext_merge_targets
