    ...
    for cell in table['cells']:
        if 'segmentation' not in cell:
            ...
            if len(segmentation) == 0:
                ...
                cell_bbox = segmentation_to_bbox(segmentation)  # (minx, miny, maxx, maxy)
...
def cal_row_span(table, cells_span, cells_bbox, height):
    ...
    for row_idx in range(layout.shape[0]):
        ...
        for cell_id in row:
            ...
            cell_bbox = cells_bbox[cell_id]  # 这里报错
        ...
        if (len(y1s) > 0) and (len(y2s) > 0):
            ...
            continue
            # raise InvalidFormat()  # gai
    rows_span = shrink_spans(rows_span, height)  # 精细一下span
...
def cal_col_span(table, cells_span, cells_bbox, width):
    ...
    for col_idx in range(layout.shape[1]):
        ...
        if (len(x1s) > 0) and (len(x2s) > 0):
            ...
            continue
            # raise InvalidFormat()  # gai
    cols_span = shrink_spans(cols_span, width)  # 对bbox的列坐标进行shrink
...
def extract_fg_bg_spans(table, image_size):
    ...
    cells_bbox = cal_cell_bbox(table)  # List of bbox
    cells_span = cal_cell_spans(table)  # List of 单元格的位置[start_row, start_col, end_row, end_col]
    # cal rows fg bg span
    # 报错原因：cells_bbox少了一个, 应该是空单元格没给bbox
    rows_fg_span, rows_bg_span = cal_row_span(table, cells_span, cells_bbox, height)  # 前景和后景行分割的ymin, ymax
    #cal cols fg bg span
